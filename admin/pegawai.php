        ...
        if ($action === 'create') {
            ...
            if (!empty($username) && !empty($password) && !empty($nama_lengkap) && !empty($jabatan_id)) {
                try {
                    ...
                    $stmt = $pdo->prepare("INSERT INTO users (username, password, role, nama_lengkap, nip, jabatan_id) VALUES (?, ?, 'staf', ?, ?, ?)");
                    $stmt->execute([$username, $hash, $nama_lengkap, $nip ? $nip : null, $jabatan_id]);
                    $success = "Pegawai '$nama_lengkap' berhasil ditambahkan!";
                } catch (PDOException $e) {
                    $error = "Gagal menambah pegawai. Username atau NIP mungkin sudah digunakan.";
                }
            } else {
            ...
            if (!empty($id) && !empty($username) && !empty($nama_lengkap) && !empty($jabatan_id)) {
                try {
                    if (!empty($password)) {
                        ...
                        $hash = password_hash($password, PASSWORD_DEFAULT);
                        $stmt = $pdo->prepare("UPDATE users SET username = ?, password = ?, nama_lengkap = ?, nip = ?, jabatan_id = ? WHERE id = ? AND role = 'staf'");
                        $stmt->execute([$username, $hash, $nama_lengkap, $nip ? $nip : null, $jabatan_id, $id]);
                    } else {
                        // Update without password
                        $stmt = $pdo->prepare("UPDATE users SET username = ?, nama_lengkap = ?, nip = ?, jabatan_id = ? WHERE id = ? AND role = 'staf'");
                        $stmt->execute([$username, $nama_lengkap, $nip ? $nip : null, $jabatan_id, $id]);
                    }
                    $success = "Data pegawai berhasil diperbarui!";
                } catch (PDOException $e) {
                    $error = "Gagal memperbarui pegawai. Username atau NIP mungkin sudah digunakan.";
                }
            } else {
                $error = "Kolom bertanda bintang (*) wajib diisi.";
            }
        ...
        <div class="modal-content rounded-3">
            ...
            <form method="POST" action="">
                ...
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        <label for="edit_password" class="form-label text-secondary fw-semibold">Password Baru (Biarkan kosong jika tidak diganti)</label>
