    ...
    if (!empty($nama_lengkap) && !empty($username)) {
        try {
            if (!empty($password)) {
                ...
                $stmt->execute([$nama_lengkap, $username, $hash, $admin_id]);
            } else {
                // Update without password
                $stmt = $pdo->prepare("UPDATE users SET nama_lengkap = ?, username = ? WHERE id = ?");
                $stmt->execute([$nama_lengkap, $username, $admin_id]);
            }
...
<div class="page-header">
    <h4 class="fw-bold text-dark mb-1">Profil Admin</h4>
    <p class="text-muted mb-0">Perbarui informasi profil dan kata sandi akun administrator Anda.</p>
</div>
        ...
        <div class="card card-custom">
            ...
            <form method="POST" action="">
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control" id="nama_lengkap" name="nama_lengkap" value="<?= htmlspecialchars($admin['nama_lengkap']) ?>" required>
                </div>
                <div class="mb-3">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control bg-light" id="nip" value="<?= htmlspecialchars($admin['nip'] ?? '-') ?>" disabled>
                    <div class="form-text text-muted">NIP tidak dapat diubah secara langsung. Hubungi developer jika ada kesalahan.</div>
                </div>
                <div class="mb-4">
    ...
    <div class="col-lg-6">
        <div class="card card-custom h-100 bg-light border-0">
            <div class="text-center py-4">
                ...
                <p class="text-muted px-4">Pastikan username dan kata sandi Anda tidak mudah ditebak untuk menjaga integritas data presensi staf Kantor Desa Sungai Rambut.</p>
            </div>
            <hr>
            <div class="p-3">
